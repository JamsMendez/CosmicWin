...
public sealed class HttpAlertCommandServerTests(ITestOutputHelper output)
{
    ...
    // ---- H5b: post-start reachability self-probe ----
    //
    // BUG (measured on hardware, 2026-09-25): a plain Winsock TcpListener already bound to the port
    // made HttpListener.Start() succeed WITHOUT throwing -- the app traced "listening", and every
    // request then hung forever, because the squatter accepted the TCP connection and simply never
    // answered it. After the squatter exited, requests got connection refused: http.sys never
    // quietly re-bound on its own. A successful, non-throwing Start() is therefore not proof this
    // instance can actually be reached -- hence the background self-probe covered below.
    //
    // MEASURED attempting to reproduce the underlying quirk from a test in THIS environment: a raw
    // Winsock TcpListener squatting the port first -- both in-process and from a genuinely separate
    // process, bound to 127.0.0.1 and to 0.0.0.0, with and without SO_REUSEADDR -- always makes
    // HttpListener.Start() throw immediately here (the existing, already-tested
    // Start_PortAlreadyInUse_ReportsDiagnosticAndDoesNotThrow path above), unlike the hardware where
    // it silently succeeded. So the one precondition this fix defends against -- Start() succeeding
    // over a port nothing of ours actually controls -- could not be produced from a black-box test
    // here. The self-probe's own decision logic and its consequences (the diagnostic wording, and
    // reacting to failure by actually closing the listener) are instead exercised below through the
    // internal, test-only selfProbeOverride seam, over a REAL HttpListener that really did start --
    // the same documented-gap idiom "R3-005" above already uses for a condition no black-box test can
    // force open.

    [Fact]
    public async Task NormalStart_SelfProbeReportsReachable_AndNeverEmitsARejectedDiagnosticForItself()
    {
        var port = GetFreePort();
        var diagnostics = new List<string>();
        using var server = new HttpAlertCommandServer(port, Token, _ => "ok", msg =>
        {
            lock (diagnostics)
            {
                diagnostics.Add(msg);
        ...
        await WaitForDiagnosticAsync(diagnostics, line => line == $"alert http: reachable on port {port}");

        lock (diagnostics)
        {
            Assert.DoesNotContain(diagnostics, l => l.StartsWith("alert-http rejected", StringComparison.Ordinal));
        }
    }

    [Fact]
    public async Task SelfProbeHeaderWithOrigin_StillReturns403_TheHeaderNeverBypassesAnEarlierGateStep()
    {
        var port = GetFreePort();
        using var server = Start(port, _ => "ok");

        using var request = new HttpRequestMessage(HttpMethod.Get, $"http://127.0.0.1:{port}{AlertHttpProtocol.AlertsPath}");
        request.Headers.TryAddWithoutValidation(HttpAlertCommandServer.SelfProbeHeaderName, "1");
        request.Headers.Add("Origin", "http://evil.example");
    ...
    [Fact]
    public async Task UnreachableSelfProbe_ReportsNotReachable_AndClosesTheListenerSoThePortComesFree()
    {
        var port = GetFreePort();
        var diagnostics = new List<string>();
        var server = new HttpAlertCommandServer(port, Token, _ => "ok",
            ...
            selfProbeOverride: _ => Task.FromResult(SelfProbeOutcome.Unreachable("simulated squatter")));

        var startException = Record.Exception(server.Start);
        Assert.Null(startException);

        var expected = $"alert http: NOT reachable on port {port} (simulated squatter); " +
            "another program may be using the port. The named pipe still works.";
        await WaitForDiagnosticAsync(diagnostics, line => line == expected);

        // Real proof of "inert", not just the diagnostic: the REAL listener this test's Start() call
        // above genuinely bound to the port was genuinely stopped and closed in reaction, so the port
        // itself comes back free -- the same proof Start_AfterDispose_IsANoOpAndLeavesThePortFree uses.
        using var probe = new TcpListener(IPAddress.Loopback, port);
        var bindException = Record.Exception(probe.Start);
        probe.Stop();
        Assert.Null(bindException);
    ...
    /// <summary>Polls up to <paramref name="timeout"/> (default 5 s, no fixed long sleep) for a
    /// diagnostic matching <paramref name="predicate"/> to show up, since the H5b self-probe above
    /// resolves asynchronously on a background task rather than synchronously inside <c>Start()</c>.
    /// </summary>
    private static async Task WaitForDiagnosticAsync(
        List<string> diagnostics, Func<string, bool> predicate, TimeSpan? timeout = null)
    {
        ...
        lock (diagnostics)
        {
            seen = string.Join(" | ", diagnostics);
        }

        Assert.Fail($"No diagnostic matched within {timeout ?? TimeSpan.FromSeconds(5)}. Seen: {seen}");
    }
