...
{
    /// <summary>
    /// Two siblings: a plain swap, sizes carried along. This is guarded on there being exactly two
    ...
    /// <summary>
    /// REWRITTEN. This asserted a SWAP for a three-child group.
    /// Three-or-more siblings fork instead: the mover pairs up
    ...
    /// reversible cycle, so repeated presses keep advancing.
    /// </para>
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// REWRITTEN. This test used to assert the opposite: that
    /// the focused node was buried in a NEW nested group together with an adjacent sibling, leaving
    /// it one level DEEPER than it started. Case (1) does the reverse -- the level
    ...
    /// <summary>
    /// REWRITTEN. The old name said it all
    ...
    /// So the node lands at index 0 for Left/Up and index 1 for Right/Down.
    /// </summary>
    [Fact]
    ...
    /// it INTO that group whenever the level holds exactly two children (<c>len == 2</c>).
